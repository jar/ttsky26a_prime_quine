module q(input k, output reg u, output reg v, output p);
  reg [9:0] x;
  reg [9:0] y;
  wire [9:0] xx = x % 5;      // x-cell (from left) presenting a character glyph
  wire [9:0] yy = y % 9;      // y-cell (from top)
  wire w = x == 799;          // end of the row
  wire h = y == 524;          // end of the frame
  wire d = x < 640 & y < 480; // draw
  reg [$clog2(3072)-1:0] i;   // glyph index
  reg [$clog2(n)-1:0] s;      // string index
  reg [$clog2(799/5)-1:0] o;  // string offset within a row
  always @(posedge k) begin
    u <= x >= 656 ~& x <= 751;  // hsync
    v <= y >= 490 ~& y <= 491;  // vsync
    x <= w ? 0 : x + 1;         // increment pixel x-coordinate
    y <= w ? h ? 0 : y + 1 : y; // increment pixel y-coordinate
    if (w) begin // at the end of a line
      o <= 0;    // always reset the in-line string offset because we have to reset until the next string line
      if (h) begin // reset everything else if at the end of the frame
        {s,i} <= 0;
      end else begin
        if (yy == 8) begin // at the end of the line and the end of bottom edge of the last cell
          s <= s + o + 1;
        end
      end
    end else if (xx[2]) begin // at the last pixel in the cell
        if (~nl && ~gl) begin // increment offset if we haven't hit a new line or we're in the glyph phase
          o <= o + 1;
        end
        if (gl) begin
          //i <= i + {4'd0,o} + 1;//XXX
          i <= i;
          //$display("i = %x", i);
        end
        //$display("si = (n-s-o-1) = %d", n-s-o-1);
        //$display("s-o = %d-%d = ", s, o, s-o);
    end
  end

  wire [3:0] l = g[3068-4*i+:4]; // 4 bits of glyph for use within the glyph hex mode
  wire [7:0] c = (~gl) ? str[8*(n-s-o-1)+:8] : (l<10) ? {4'd0,l}+48 : {4'd0,l}+87; // switch between string and glyph hex (0...f)
  wire nl = c == 35; // using '#' in place of newline character. In case we need capital letters
  wire gl = s+o == ny; // position of glyph hex data. Positional rather than a capital character
  wire [8:0] gy = 4 * (c - 32) + {7'd0, xx[1:0]}; // offset 32 unprinted ASCII
  assign p = (xx[2] || yy == 8 || nl) ? 0 : g[384*yy[2:0]+gy] & d;

wire [3071:0] g = 3072';
parameter n = 270;
parameter ny = n-24;
wire [8*n-1:0] str = "module q(input k, output reg u, output reg v, output p);#  reg [9:0] x;#  reg [9:0] y;#  wire w = x == 799;#  wire h = y == 524;#  wire [7:0] c = (~gl) ? str[8*(n-s-o-1)+:8] : (l<10) ? {4'd0,l}+48 : {4'd0,l}+87;#endmodule#wire [3071:0] g = 3071'h;#wire [8*n-1:0] str = ;";
//parameter n = 96;
//parameter [8*n-1:0] str = {" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~\177"};

endmodule
