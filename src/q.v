module q(input k, output reg u, output reg v, output p);
  reg [9:0] x, y;
  wire [9:0] xx = x % 5, yy = y % 9; // x-cell (from left) presenting a character glyph, y-cell (from top)
  wire [12:0] z = 128 * (y/9) + (x/5); // cell
  ...
  wire gl = (z >= i && z < i+768); // position of glyph hex data
  wire [3:0] l = g[(3068-4*(z-i))+:4]; // 4 bits of glyph for use within the glyph hex mode
  wire [7:0] c = gl ? (l<10) ? {4'd0,l}+48 : {4'd0,l}+87 : a[8*(n-z-1)+:8]; // switch between string and glyph hex (0...f)
